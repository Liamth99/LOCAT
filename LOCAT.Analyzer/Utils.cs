    ...
    public static string? GetNamespaceOfClass(ClassDeclarationSyntax classDeclaration)
    {
        ...
        while (parent != null)
        {
            if (parent is NamespaceDeclarationSyntax namespaceDeclaration)
            {
                ...
                namespaceName = namespaceDeclaration.Name + (namespaceName == null ? string.Empty : "." + namespaceName);
            }
            else if (parent is FileScopedNamespaceDeclarationSyntax fileScopedNamespace)
            {
                // Handle file-scoped namespaces
                namespaceName = fileScopedNamespace.Name + (namespaceName == null ? string.Empty : "." + namespaceName);
            }
